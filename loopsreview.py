...
for i in range(26):
  if alphabet[i] in "aeiou":
    print(alphabet[i].upper() + " is a vowel.")
  else:
    print(alphabet[i].upper() + " is a consonant.")
...
def RabbitsMultiplying(yrs):
  total = 0
  females = 1
  reproductive_pool = 1
  for a in range(yrs):
    reproductive_pool = females
    for b in range(reproductive_pool*12):
...
def times_table(top):
  output = '* |'
  [output += '  '+str(i) for i in range(1, top+1)]
  output += '\n-'+'-'*len(output)
...
def square_and_x():
  print(line([i for i in range(7)]))
  for i in range(3):
    print(line([0, 1-i, 5-i, 6]))
  for i in reversed(range(2)):
    print(line([0, 2-i, 4-i, 6]))
  print(line([i for i in range(7)]))
...
'''
integer = 234879623047357890

[print(i+' ', end='') for i in str(integer)[::-1]]
